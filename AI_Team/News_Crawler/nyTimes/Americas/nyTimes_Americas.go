...
func main() {
	...
	article_amount, err := ioutil.ReadFile("Americas/article_amount.txt")
	panicError(err)
	...
	for t := time.Tick(time.Duration(updateTime) * time.Minute); ; <-t {
		...
		crawlTimes++
		fmt.Println("==============================================================")
		fmt.Println("nyTimes_Americas Crawl Times: ", crawlTimes)
		end := time.Since(start)
		log.Printf("nyTimes_Americas Time: %s", end)
		fmt.Println("==============================================================")
	}
}
