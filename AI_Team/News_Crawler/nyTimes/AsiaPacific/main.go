...
func main() {
	...
	article_amount, err := ioutil.ReadFile("article_amount.txt")
	panicError(err)
	...
	for t := time.Tick(time.Duration(updateTime) * time.Minute); ; <-t {
		for _, article := range feed.Items {
			...
			if (len(content.String()) != 0 && !db_helper.ArticleExist(db, hash_url)) {
				...
				writeToFile("article_amount.txt", stringCountArticle)
				...
				fmt.Println("INSERT DATABASE: FAIL, Article Existed!")
			}
