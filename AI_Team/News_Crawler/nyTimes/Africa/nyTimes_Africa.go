...
func main() {
	url_rss := "http://rss.nytimes.com/services/xml/rss/nyt/Africa.xml"
	...
	article_amount, err := ioutil.ReadFile("Africa/article_amount.txt")
	panicError(err)
	...
	for t := time.Tick(time.Duration(updateTime) * time.Minute); ; <-t {
		for _, article := range feed.Items {
			...
			if (len(content.String()) != 0 && !db_helper.ArticleExist(db, hash_url)) {
				...
				writeToFile("Africa/article_amount.txt", stringCountArticle)
				err = db_helper.InsertAfricaTable(db, date, topic, title, introduction, content, article.Link, hash_url)
				if err != nil {
					fmt.Println("INSERT nyTimes_Africa DATABASE: ERROR")
					fmt.Println(err.Error())
					//panic(err.Error())
				} else {
					fmt.Println("INSERT nyTimes_Africa DATABASE: OK")
				}
			} else if (db_helper.ArticleExist(db, hash_url)) {
				fmt.Println("INSERT nyTimes_Africa DATABASE: FAIL, Article Existed!")
			}
			fmt.Println("nyTimes_Africa Article Amount:", countArticle)
		}

		crawlTimes++
		fmt.Println("==============================================================")
		fmt.Println("nyTimes_Africa Crawl Times: ", crawlTimes)
		end := time.Since(start)
		log.Printf("nyTimes_Africa Time: %s", end)
		fmt.Println("==============================================================")
	}
}
